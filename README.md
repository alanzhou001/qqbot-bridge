...
当前本机 OpenClaw 已配置为本机 Gateway：`127.0.0.1:18789`，默认本地记忆检索 embedding 走 Ollama `qwen3-embedding:0.6b`。Bridge 不接入 OpenClaw 的 QQ channel，只调用固定 `qqbot` agent。
...
```text
/Volumes/File/qqbot-data/knowledge/
/Volumes/File/qqbot-data/rag_context/
/Volumes/File/qqbot-data/rag_index/
/Volumes/File/qqbot-data/openclaw/workspaces/qqbot/
```
...
数据库默认写入 `/Volumes/File/qqbot-data/qqbot.sqlite3`。如果 File 盘路径变化，改 `.env` 中的 `DATA_DIR` 和 `DB_PATH`。
...
- Prompt 上下文文件：`/Volumes/File/qqbot-data/rag_context/*.md`，适合先由本地 RAG 进程生成短摘要，再交给 OpenClaw。

Bridge 当前会按顺序读取：

```text
/Volumes/File/qqbot-data/rag_context/shared.md
/Volumes/File/qqbot-data/rag_context/groups/<群号>.md
/Volumes/File/qqbot-data/rag_context/users/<QQ号>.md
```

读取到的内容会被限制在 `RAG_CONTEXT_MAX_CHARS` 内，再注入 `qqbot` prompt。这样后续本地 RAG 可以独立演进，不需要把 QQ 协议层塞进 OpenClaw Gateway。

新增或更新 `/Volumes/File/qqbot-data/knowledge/` 下的录取资料后，运行：

```bash
python scripts/update-knowledge-index.py
```

这会刷新 `/Volumes/File/qqbot-data/rag_context/shared.md`，让 `qqbot` 知道可查阅的本地资料路径。

如果要把投档线/省控线导入 SQLite，并把非表格资料用本地 Qwen embedding 写入向量库，运行：

```bash
/Users/alanz/.cache/codex-runtimes/codex-primary-runtime/dependencies/python/bin/python3 scripts/import-knowledge.py
```
